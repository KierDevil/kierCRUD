...
- Preserve the existing MySQL default: `Server=localhost;Port=3306;Database=kiercrud;User=root;Password=...`. Read connection settings from configuration or `DB_CONNECTION_STRING` when database behavior needs to be configurable, and do not expose credentials in logs or error responses.
